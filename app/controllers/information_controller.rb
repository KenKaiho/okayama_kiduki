class InformationController < ApplicationController
  ...
  def show
    sql="select * from awarenesses left join awarenesses_has_informations on awarenesses_id=id where information_id=?"
    @awareness=Awareness.find_by_sql([sql,params[:id]])
  end
  ...
  def create
    ...
    @information.insert_time=Time.now
    @information.update_time=Time.now
  ...
  private
    ...
    def information_params
      params.require(:information).permit(:title, :url, :comment, :insert_time, :update_time,:users_id)
    end
end
